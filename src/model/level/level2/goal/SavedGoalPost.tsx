...
function SavedGoalPost({ calls, state, projectionMode, s__projectionMode }: any) {
  ...
  return (<>
    {state.hasAnyToken &&
      <group position={[0.5, 0, 1.95]}>
        <Cylinder args={[0.25, 0.38, 0.75, 6]} position={[0, -0.32, 0]}
          castShadow receiveShadow material={StandardColor("#cccccc")}
        />
      </group>
    }
    {state.hasAnyToken && <>
      <group position={[0.35, -0.55, 1.95]}>
        {userDatabaseArray.filter((aTrade: any, index: number) =>
          (aTrade.profitLoss > 0)).slice(0, 5).map((anOrder: any, index: any) => {
            return (
              <Box args={[0.065, 0.1, 0.18]} position={[index * 0.075, 0.6, 0]} castShadow receiveShadow
                key={index} material={StandardColor(anOrder.profitLoss == 0 ? "#aaaaaa" : "#ffaa33")}
              />
            )
          }
        )}
    ...
    {state.hasAnyToken && !!superuser && <>
      <group position={[0.15, 0, 2.25]} rotation={[Math.PI / 2, 0, Math.PI / 2]}>
        <DynaText text={"API STORAGE STATION"} color={"#888"} font={0.09}
          position={[-0.3, 0.46, 0.6]} rotation={[-Math.PI / 2, 0, 0]}
        />
        <DynaText text={"SYNC \n API KEYS!"} color={"#cc6600"} font={0.05}
          position={[0, 0.505, 1.04]}
          rotation={[-Math.PI / 2, 0, 0]}
        />
        <Cylinder args={[0.15, 0.15, 0.1, 4]} position={[0, 0.45, 1.05]} castShadow receiveShadow 
          ref={$claimButton} onClick={setAPIKeys} material={StandardColor("#a0dFf3")}
        />
      </group>
    </>}
    {state.hasAnyToken && !!superuser && <>
      <group position={[0.5, 0, 2.01]} rotation={[Math.PI / 2, 0, 0]}>
        {!!superuser.subscription && <>
          <Cylinder args={[0.12, 0.12, 0.1, projectionMode ? 4 : 3]} position={[0, 0.25, 0.15]} 
            castShadow receiveShadow ref={$claimButton}  onClick={() => { toggleProjection() }}
            material={StandardColor(!projectionMode ? "#a0dFf3" : "#f93")}
          />
        </>}
    ...
    {userDatabaseArray.length > 0 && <group position={[0, 0, -0.8]}>
      <DynaText text={userDatabaseArray.length} color={"#666666"} font={0.24}
        position={[0.03, -.459, 1]} rotation={[-Math.PI / 2, 0, 0]}
      />
      <DynaText text={userDatabaseArray.length == 1 ? "Trade" : "Trades"} color={"#666666"} font={0.05}
        position={[0.02, -.459, 1.15]} rotation={[-Math.PI / 2, 0, 0]}
      />
    </group>}
    ...
    {databaseLossCount > 0 && <group position={[0, 0, -0.3]}>
      ...
      <DynaText text={databaseLossCount == 1 ? "Loss" : "Losses"} color={"#cc0000"} font={0.06}
        position={[0.02, -.459, 1.07]} rotation={[-Math.PI / 2, 0, 0]}
      />
    </group>}
    ...
    {databaseProfitCount > 0 && <>
      <DynaText text={databaseProfitCount} color={"#ff9933"} font={0.2}
        position={[0.03, -.459, 1.09]} rotation={[-Math.PI / 2, 0, 0]}
      />
      <DynaText text={databaseProfitCount == 1 ? "Profit" : "Profits"} color={"#ff9933"} font={0.07}
        position={[0.04, -.459, 1.2]} rotation={[-Math.PI / 2, 0, 0]}
      />
    </>}
    ...
    {!!superuser && superuser.subscription && <>
      <group rotation={[0, Math.PI / 2, 0]} position={[1.31, 0, 2.3]}>
        <DynaText text={superuser.subscription} color={"#33cc33"}
          position={[0, -0.85, 0]} rotation={[0, 0, 0]}
        />
        <DynaText text={superuser.subscription == 1 ? "City Level" : "Level"} 
          color={"#66aa66"} font={0.07}
          position={[0.0, -0.6, 0]} rotation={[0, 0, 0]}
        />
      </group>
      <group rotation={[0, Math.PI / 2, 0]} position={[1.31, 0, 1.95]}>
        <DynaText text={superuser.goodAttempts} color={"#ffaa33"}
          position={[0, -0.85, 0]} rotation={[0, 0, 0]}
        />
        <DynaText text={superuser.goodAttempts == 1 ? "Star" : "Stars"} 
          color={superuser.goodAttempts > 0 ? "#ffaa33" : "#777777"} font={0.07}
      ...
      <group rotation={[0, Math.PI / 2, 0]} position={[1.31, 0, 1.63]}>
        <DynaText text={superuser.attempts} color={"#3366ff"} font={0.16}
          position={[0.23, -0.6, 0]} rotation={[0, 0, 0]}
        />
        <DynaText text={superuser.attempts == 1 ? "Tickets:" : "Tickets:"} 
          color={superuser.attempts > 0 ? "#3366ff" : "#777777"} font={0.07}
          position={[0, -0.6, 0]} rotation={[0, 0, 0]}
        />
      </group>
    </>}
